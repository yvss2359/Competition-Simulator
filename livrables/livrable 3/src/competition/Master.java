...
// A master is a type of competition that is played in two phases, the pool phase(League), and then the tournament between the pool phase winners.
public class Master extends Competition {
	private Strategy strat;
	private int groups;
	
	//Master constructor
	public Master(List<Competitor> competitors,Match match,List<Observer> ob, Strategy strat,int groups){
	...
	protected void play(List<Competitor> competitors) {
		...
		Competition finals = new Tournament (finalc, match, this.getObservers());
		finals.play(finalc);
	...
	public List<Competitor> poolPhase (List<Competitor> competitors){
		...
		for(List<Competitor> l: comps) {
			leagues.add(new League(l, new MatchRandom(),this.getObservers()));
		}
